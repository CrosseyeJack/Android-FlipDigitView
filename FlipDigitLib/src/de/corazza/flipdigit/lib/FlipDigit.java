...
class FlipDigit implements AnimationListener {
	...
	private static final int ALLNUMBERS = 0;
	...
	private void init() {
		...
		animation2.setStartOffset(Math.round((context.getResources().getInteger(R.integer.duration_point_flip_animation)*0.68)));
	...
	private void incrementFromCode() {
		animateFrom = animateFrom + count;
		if (animateFrom < 0) {
			animateFrom = count == 1 ? 0 : 9;
		}

		if (animateFrom > 9) {
			animateFrom = count == 1 ? 9 : 0;
		}
	...
	private int getDigitToShow() {
		if(flipMode == ALLNUMBERS) {
			if (animateFrom + count > 9) {
				return 0;
			}else if(animateFrom + count < 0) {
				return 9;
			} else {
				return animateFrom + count;
			}
		}else {
			return animateTo;
		}
